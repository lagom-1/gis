...
def _geojson_to_ee_geometry_local(geojson: Dict[str, Any]) -> ee.Geometry:
    """兼容旧调用，委托给 gis.gee.geometry"""
    from gis.gee.geometry import geojson_to_ee_geometry
    return geojson_to_ee_geometry(geojson)


def _normalize_region(region: Any = None, region_path: Optional[str] = None):
    """兼容旧调用，委托给 gis.gee.geometry"""
    from gis.gee.geometry import normalize_region
    return normalize_region(region=region, region_path=region_path)
